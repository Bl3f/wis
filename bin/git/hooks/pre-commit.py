#!/usr/bin/env python
# Source: https://gist.github.com/1376856
# From root dir execute :
# "cd .git/hooks/ && rm -f pre-commit && ln -s ../../bin/git/hooks/pre-commit.py pre-commit && cd ../../"
...
def main(all_files):
    ...
    p = subprocess.Popen('pep8 --max-line-length=119  web/',
                         shell=True, stdout=subprocess.PIPE)
    out, _ = p.communicate()
    pep8 = out.splitlines()
    if len(pep8) > 0:  # When all is OK for pep8, output = 2 lines.
        for line in pep8:
            print line
        #exit_on_err(1)
        return_code = 1
